...
class CharacterizationProtocol(vxprotocol.StaticProtocol):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # PART 3b: On/Off Steps (no gray)
        repeats = 5
        for i in range(repeats):
            for color in onoff2_color:
                p = vxprotocol.Phase(onoff2_phase_dur)
                p.set_visual(SphereUniformBackground,
                             {SphereUniformBackground.u_color: np.array([color, color, color])})
                self.add_phase(p)
