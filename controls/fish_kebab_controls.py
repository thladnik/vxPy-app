...
class Control02(vxcontrol.BaseControl):
    ...
    def main(self, dt, **pins):
        ...
        if self.current_time < self.led_start_state_duration:
            self.led_left_current_state += (((self.led_left_state - self.led_left_start_state) / self.led_start_state_duration) * dt)
            self.led_right_current_state += (((self.led_right_state - self.led_right_start_state) / self.led_start_state_duration) * dt)

        elif self.current_time > self.led_start_state_duration + self.led_middle_state_duration:
            self.led_left_current_state += (((self.led_left_end_state - self.led_left_state) / self.led_end_state_duration) * dt)
            self.led_right_current_state += (((self.led_right_end_state - self.led_right_state) / self.led_end_state_duration) * dt)
...
class ControlSinusoidal(vxcontrol.BaseControl):
    ...
    def initialize(self, **kwargs):
        ...
        # Add datasets to be saved to file
        vxcontainer.add_phase_attributes({'frequency': self.frequency, 'amplitude': self.amplitude,
                                              'rounds': self.rounds, 'steps_to_full_rotation': self.steps_to_full_rotation})
        vxcontainer.create_phase_dataset('led_left', (1,), np.float32)
        vxcontainer.create_phase_dataset('led_right', (1,), np.float32)
    ...
    def main(self, dt, **pins):
        ...
        # self.device.board.stepper_get_current_position(self.device.motor, self._callback_current_pos)

        target_pos = int(self.amplitude * np.sin((2 * np.pi * self.frequency * self.delta_t)) * (self.steps_to_full_rotation / 360))

        # self.device.board.stepper_move(self.device.motor, target_pos - self.current_pos)

        self.device.board.stepper_move_to(self.device.motor, target_pos)
