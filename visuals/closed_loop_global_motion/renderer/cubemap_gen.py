...
class MyApp(ShowBase):
    ...
    def move_actor_fish(self, task):
        ...
        translational_speed = np.float64(fish_motion_vec[0])

        # their 0 axis for angles is the y axis. We use the x axis so always subtract 90
        angular_speed = np.float64(fish_motion_vec[1]) * 180 / np.pi

        # # debug
        # translational_speed = 1e-1
        # angular_speed = np.pi/2
        # print(f'cubemap swim angle {angular_speed}')
        #print(f'cubemap fish v vec y {fish_motion_vec[1]}')
        current_ori = self.main_actor.getH()

        # update orientation
        updated_ori_deg = current_ori + rot_scale_factor * angular_speed * dt
        self.main_actor.setH(updated_ori_deg)


        # Translation
        trans_scale_factor = 1000 # adjust this if you want the fish to swim faster with the same bout
        pos = self.main_actor.getPos()
        # again the 0 angle axis is the y axis in the world. Subtract 90 degrees from sin/cos arg
        delta_x = -np.sin(updated_ori_deg * np.pi / 180) * trans_scale_factor * translational_speed * dt
        delta_y = np.cos(updated_ori_deg * np.pi / 180) * trans_scale_factor * translational_speed * dt
        translation_vec = np.array([delta_x,delta_y,0])

        # add some pushback to the fish
        add_external_translation = False # set to True if you want fish to be flooded backwards
        if add_external_translation:
            translation_vec -= 2*dt if time.perf_counter() % 2 > 1 else 0

        translation = LPoint3f(*[v for v in translation_vec])
...
def run(raw_data, _fish_motion_vec, _x, _y, _heading, _fb_size):
    global data, fish_motion_vec, fb_size, x_pos, y_pos, heading
    fb_size = _fb_size
    fish_motion_vec = _fish_motion_vec
    x_pos = _x
    y_pos = _y
    heading = _heading
