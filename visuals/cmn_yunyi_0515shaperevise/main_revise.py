...
class CMN_FORE_BACK(vxvisual.SphericalVisual):
    ...
    cmn_time = vxvisual.FloatParameter('cmn_time', internal=True)
    ...
    def __init__(self, *args, **kwargs):
        ...
        for b in range(self.CMN_phase):
            for i in range(5 * self.fps):  # cmn_duration = 5
                stacked_quats_foreground_nextframe = self.rotation_quats[i] * stacked_quats_foreground
                stacked_quats_foreground = qt.slerp(stacked_quats_foreground, stacked_quats_foreground_nextframe,
                                                    1 / self.fps)
                stacked_quats_nextframe = self.rotation_quats[i] * stacked_quats_currentframe
                stacked_quats_currentframe = qt.slerp(stacked_quats_currentframe, stacked_quats_nextframe, 1 / self.fps)
        self.stacked_quats_foreground_start = stacked_quats_foreground
        self.foreground_cmn_current_quats = stacked_quats_foreground
        self.stacked_quats_foreground_start = np.repeat(self.stacked_quats_foreground_start[:, None], 3,
    ...
    def initialize(self, **kwargs):
        ...
        self.cmn_time.data = self.CMN_phase * 5.
    def render(self, dt):

        # Update time
        self.time.data += dt
        self.cmn_time.data += dt
        # Get correct time index
        tidx = int(self.time.data[0] * self.fps) % self.frame_num
        tidx_cmn = int(self.cmn_time.data[0] * self.fps) % self.frame_num
        ...
        fore_cmn_future_quats = self.rotation_quats[tidx_cmn] * self.foreground_cmn_current_quats
