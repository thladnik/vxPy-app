...
class Triangle(vxvisual.SphericalVisual):
    ...
    # Paths to shaders
    VERT_PATH = 'position_fore_back.vert'
    FRAG_PATH = 'texture_fore_back.frag'
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.sphere = sphere.ForeBackSphere(azim_lvls=60, elev_lvls=30, upper_elev=np.pi / 2, subdivisionTimes=2)
        ...
        self.texture_start_coords = Geometry.cen2tri(np.random.rand(num), np.random.rand(num), .1)
        self.texture_coords = np.float32(self.texture_start_coords.reshape([-1, 2]) / 2)
        # self.texture_coords_buffer = gloo.VertexBuffer(self.sphere.get_uv_coordinates())
    ...
    def initialize(self, **params):
        ...
        num = self.sphere.azim_lvls * self.sphere.elev_lvls
        self.texture_start_coords = Geometry.cen2tri(np.random.rand(int(num/3)), np.random.rand(int(num/3)), .1)
