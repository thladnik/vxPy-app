...
uniform float time;
uniform float sphere_texture_u_scale;
uniform float sphere_texture_v_scale;
...
void main() {
      ...
      if(v_tile_center_x >= -1.9031870  && v_tile_center_x <= -1.2384055 && v_tile_center_y <= -0.08800039 && v_tile_center_y >= -0.45215960){
         ...
         float sphere_u = (1.0+v_azimuth/PI)/2.0 ;
         sphere_u = sphere_u / sphere_texture_u_scale + v_sphere_texture_start_coords.x+ deltaU;
         float sphere_v = (1.0+v_elevation*2.0/PI)/2.0;
         sphere_v = sphere_v / sphere_texture_v_scale + v_sphere_texture_start_coords.y;
