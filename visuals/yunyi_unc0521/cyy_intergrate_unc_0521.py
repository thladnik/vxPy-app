...
import visuals.yunyi_unc0521.cyy_sphere as sphere
import math
...
class CMN_ForegroundBackground(vxvisual.SphericalVisual):
    ...
    def __init__(self, *args, **kwargs):
        ...
        texture_triangle_size = .1
        texture_triangle_edge_size = texture_triangle_size * np.sqrt(3)
        self.texture_start_coords = Geometry.cen2tri(np.random.rand(face_num), np.random.rand(face_num), texture_triangle_size)
        ...
        random_texture_scale = self.sphere.triangle_size/texture_triangle_edge_size
        sphere_texture_u_scale = 2 * math.pi/random_texture_scale
        self.rotating_dot['sphere_texture_u_scale'] = sphere_texture_u_scale
        sphere_texture_v_scale = math.pi/random_texture_scale
        self.rotating_dot['sphere_texture_v_scale'] = sphere_texture_v_scale
        print(f'{self.sphere.triangle_size=}')
        print(f'{texture_triangle_edge_size=}')
        print(f'{random_texture_scale=}')
        print(f'{sphere_texture_u_scale=}')
        print(f'{sphere_texture_v_scale=}')

        sphere_texture_start_coords = np.random.rand(face_num, 2)
        self.sphere_texture_start_coords = np.float32(np.repeat(sphere_texture_start_coords, 3, axis=0))
        print(f'{self.sphere_texture_start_coords=}')
        self.rotating_dot['a_sphere_texture_start_coords'] = gloo.VertexBuffer(self.sphere_texture_start_coords)
...
class ForegroundMovingForward(CMN_ForegroundBackground):
    VERT_PATH = 'foreground_moving_forward_vert.vert'
    FRAG_PATH = 'foreground_moving_forward(1).frag'
...
if __name__ == '__main__':
    ...
    # print(texture)
    # s = sphere.IcoSphere_UVsphere_new(subdivisionTimes=0)
    # print(f'{s.triangle_size=}')
    # texture_triangle_size = .1
    # texture_triangle_edge_size = texture_triangle_size * np.sqrt(3)
    # random_texture_scale = s.triangle_size / texture_triangle_edge_size
    # sphere_texture_u_scale = 2 * math.pi / random_texture_scale
    # sphere_texture_v_scale = math.pi / random_texture_scale
    # print(f'{texture_triangle_edge_size=}')
    # print(f'{random_texture_scale=}')
    ...
    face_num = 4
    sphere_texture_start_coord = np.random.rand(face_num, 2)
    print(f'{sphere_texture_start_coord=}')
    sphere_texture_start_coord = np.repeat(sphere_texture_start_coord,3, axis=0)
    print(f'{sphere_texture_start_coord=}')
