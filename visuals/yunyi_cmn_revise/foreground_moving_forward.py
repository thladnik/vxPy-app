...
class ForegroundMovingForward(vxvisual.SphericalVisual):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.texture_coords_eachframe = self.texture_coords
        for i in range(1, 15*self.stimulus_fps+1):
            motmat_eachframe = np.repeat(self.rotated_motion_matrix[:, i], 3, axis=0)
            self.texture_coords_eachframe += np.array([np.real(motmat_eachframe), np.imag(motmat_eachframe)]).T * self.norm_speed
    ...
    def initialize(self, **params):
        ...
        # self.texture_coords = np.float32(self.texture_start_coords.reshape([-1, 2]) / 2)
        self.texture_coords = self.texture_coords_eachframe
    ...
    def render(self, dt):
        ...
        if frame_idx > self.frame_index.data[0]:
            # frame_idx = 30 * self.stimulus_fps + frame_idx
