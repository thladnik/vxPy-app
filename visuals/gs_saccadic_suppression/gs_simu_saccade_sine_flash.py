...
class Luminance(vxvisual.FloatParameter):
    ...
    def upstream_updated(self):
        ...
        time = SimuSaccadeWithSineFlash.time.data[0]
        lum_amp = SimuSaccadeWithSineFlash.sine_amp.data[0]
        sine_freq = SimuSaccadeWithSineFlash.sine_freq.data[0]
        lum_mean = SimuSaccadeWithSineFlash.baseline_lum.data[0]

        lum = lum_mean + np.sin(sine_freq * time * 2.0 * np.pi) * lum_amp / 2.0
        self.data = lum


class SimuSaccadeWithSineFlash(vxvisual.SphericalVisual):
